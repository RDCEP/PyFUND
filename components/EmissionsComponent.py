...
class EmissionsComponent(Behaviors):
    ...
    def run(self, state, clock, dimensions):
        ...
        if (clock.IsFirstTimestep):
            ...
            s.mco2[t] = (globco2)
            s.globch4[t] = (max(0.0, globch4 + ((t - clock.first)
                                                > 50 and s.ch4add * ((t - clock.first) - 50) or 0.0)))
            s.globn2o[t] = (max(
                0.0, globn2o + ((t - clock.first) > 50 and s.n2oadd * ((t - clock.first) - 50) or 0)))
            s.globsf6[t] = (max(0.0, globsf6 + ((t - clock.first)
                                                > 50 and s.sf6add * ((t - clock.first) - 50) or 0.0)))
            s.globso2[t] = (globso2)
