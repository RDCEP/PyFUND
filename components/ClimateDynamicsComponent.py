...
# 2013-11-05 21:16:10.790503. Think long and hard before
...
class IClimateDynamicsState(Parameters):
    radforc = IParameter1Dimensional(
        'radforc', ['Timestep'], 'double', 'Total radiative forcing')
    temp = IVariable1Dimensional(
        'temp', ['Timestep'], 'double', 'Average global temperature')
...
class ClimateDynamicsComponent(Behaviors):
    ...
    def run(self, state, clock, dimensions):
        ...
        if (clock.IsFirstTimestep):
            ...
            LifeTemp = (max(s.LifeTempConst + s.LifeTempLin * s.ClimateSensitivity +
                        s.LifeTempQd * math.pow(s.ClimateSensitivity, 2.0), 1.0))
