...
# 2013-10-25 13:48:08.306966. Think long and hard before
...
class IScenarioUncertaintyState(Parameters):
    ...
    timeofuncertaintystart = ScalarVariable(
        'timeofuncertaintystart', 'Timestep', None)

    options = [pgrowth, ypcgrowth, aeei, acei,
               forestemm, scenpgrowth, scenypcgrowth, scenaeei, scenacei, scenforestemm, ecgradd, pgadd, aeeiadd, aceiadd, foremadd, timeofuncertaintystart]
...
class ScenarioUncertaintyComponent(Behaviors):
    ...
    def run(self, state, clock, dimensions):
        ...
        for r in dimensions.GetValuesOfRegion():
            ...
            s.ypcgrowth[t, r] = (
                s.scenypcgrowth[t, r] + (t >= s.timeofuncertaintystart and s.ecgradd[r] * sdTimeFactor or 0.0))
            s.pgrowth[t, r] = (
                s.scenpgrowth[t, r] + (t >= s.timeofuncertaintystart and s.pgadd[r] * sdTimeFactor or 0.0))
            s.aeei[t, r] = (
                s.scenaeei[t, r] + (t >= s.timeofuncertaintystart and s.aeeiadd[r] * sdTimeFactor or 0.0))
            s.acei[t, r] = (
                s.scenacei[t, r] + (t >= s.timeofuncertaintystart and s.aceiadd[r] * sdTimeFactor or 0.0))
            s.forestemm[t, r] = (
                s.scenforestemm[t, r] + (t >= s.timeofuncertaintystart and s.foremadd[r] * sdTimeFactor or 0.0))
