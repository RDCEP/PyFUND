...
class ISocioEconomicState(Parameters):
    income = IVariable2Dimensional('income', ['Timestep', 'Region'], 'double', None)
    consumption = IVariable2Dimensional('consumption', ['Timestep', 'Region'], 'double', None)
    ypc = IVariable2Dimensional('ypc', ['Timestep', 'Region'], 'double', None)
    ygrowth = IVariable2Dimensional('ygrowth', ['Timestep', 'Region'], 'double', None)
    plus = IVariable2Dimensional('plus', ['Timestep', 'Region'], 'double', None)
    urbpop = IVariable2Dimensional('urbpop', ['Timestep', 'Region'], 'double', None)
    popdens = IVariable2Dimensional('popdens', ['Timestep', 'Region'], 'double', None)
    globalconsumption = IVariable1Dimensional('globalconsumption',['Timestep'],'double',None)
    globalypc = IVariable1Dimensional('globalypc',['Timestep'],'double',None)
    globalincome = IVariable1Dimensional('globalincome',['Timestep'],'double',None)
    ...
    eloss = IParameter2Dimensional('eloss', ['Timestep', 'Region'], 'double', None)
    sloss = IParameter2Dimensional('sloss', ['Timestep', 'Region'], 'double', None)
    mitigationcost = IParameter2Dimensional('mitigationcost', ['Timestep', 'Region'], 'double', None)
    area = IParameter2Dimensional('area', ['Timestep', 'Region'], 'double', None)
    globalpopulation = IParameter1Dimensional('globalpopulation',['Timestep'],'double',None)
    population = IParameter2Dimensional('population', ['Timestep', 'Region'], 'double', None)
    populationin1 = IParameter2Dimensional('populationin1', ['Timestep', 'Region'], 'double', None)
...
class SocioEconomicComponent(Behaviors):
    ...
    def run(self, state, clock, dimensions):
        ...
        if (clock.IsFirstTimestep):
            ...
            s.globalconsumption[t] = (dimensions.GetValuesOfRegion().Select(lambda r: s.consumption[t, r]).Sum())
            ...
            for r in dimensions.GetValuesOfRegion():
                oldincome = (s.income[t-1,r] - ((t >= Timestep.FromSimulationYear(40)) and not
                                s.runwithoutdamage and s.consleak * s.eloss[t-1,r] / 10.0 or 0))
            ...
            for r in dimensions.GetValuesOfRegion():
                ...
                s.consumption[t, r] = (max(s.income[t, r] * 1000000000.0 * (1.0 - savingsrate) - (
                        s.runwithoutdamage and 0.0 or (s.eloss[t - 1, r] + s.sloss[t - 1, r]) * 1000000000.0), 0.0))
            ...
            for r in dimensions.GetValuesOfRegion():
                ...
                s.urbpop[t, r] = ((0.031 * math.sqrt(s.ypc[t, r]) - 0.011 * math.sqrt(s.popdens[t, r])) /
                                (1.0 + 0.031 * math.sqrt(s.ypc[t, r]) - 0.011 * math.sqrt(s.popdens[t, r])) /
                                (1 + s.urbcorr[r] / (1 + 0.001 * math.pow(Convert.ToDouble(t - clock.first) -
                                40.0, 2))))
